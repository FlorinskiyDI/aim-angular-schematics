import {
  ...
  const getOutputPath = async (
    ...
    let { outputPath } = await context.getTargetOptions(
      targetFromTargetString(buildTargetName)
    );
  
    if (typeof outputPath !== 'string') {
      throw new Error(`Invalid outputPath for target: ${buildTargetName}. Expected a string, but got ${typeof outputPath}`);
    }

    const sourceFolder = builderConfig.sourceFolder;
    if (sourceFolder !== null && sourceFolder !== undefined){
        outputPath = outputPath + "/" + sourceFolder;
    }
  ...
  export default createBuilder(
    async (
      ...
      if (builderConfig.noBuild) {
        ...
        // );
        const outputPath = await getOutputPath(buildTarget.name, context, builderConfig);
      ...
      if (buildResult.success) {
        ...
        // throw new Error(
        //     `11111111111111111111. ${JSON.stringify(outputPath)}`
        //   );
